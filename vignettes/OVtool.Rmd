...
* `progress`: whether or not the function progress should print to screen. The default value is TRUE. If the user doe not want the output to print to screen, they should set this parameter to FALSE.
...
Please see Burgette et al. (in progress) for additional details on the theory behind `OVtool`.
...
* `p_contours`:  a vector of p-value contours plotted. The default is 0.01, 0.05, and 0.1. We only recommend changing from teh default if the p-value for the raw effect is close to one of these values. This parameter is only used when `print_graphic` equals "2" or "3".
