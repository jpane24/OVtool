...
The next step is to estimate the observed treatment effect. The `outcome_model` function in `OVtool` automatically uses the `svydesign` and `svyglm` functions from the `survey` package in R ([survey](http://cran.fhcrc.org/web/packages/survey/vignettes/survey.pdf)) and formats and returns a list of components that are needed throughout step 3.
